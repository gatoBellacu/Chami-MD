...
let pluginFolder = path.join(__dirname, 'funciones')
let pluginFilter = filename => /\.js$/.test(filename)
global.funciones = {}
for (let filename of fs.readdirSync(pluginFolder).filter(pluginFilter)) {
  try {
    global.funciones[filename] = require(path.join(pluginFolder, filename))
  } catch (e) {
    conn.logger.error(e)
...
global.reload = (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (err) conn.logger.error(`syntax error while loading '${filename}'\n${err}`)
      ...
      global.funciones[filename] = require(dir)
    } catch (e) {
      conn.logger.error(e)
    } finally {
...
fs.watch(path.join(__dirname, 'funciones'), global.reload)
