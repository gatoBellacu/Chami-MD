...
if (!args[0]) throw `*⚠️ Ingrese un enlace de facebook , Ejemplo: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/` 
if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*⚠️ Ingrese un enlace de facebook, ejemplo: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`
try {
...
conn.sendFile(m.chat, r2es, 'error.mp4', `*Aqui esta tu video*`, m);
} catch (err1) {
console.log('1 ' + err1)   
try {  
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `*Aqui esta tu video*`, m)    
} catch (err2) {
console.log('2 ' + err2)    
try {
let ress = await fg.fbdl(args[0])
let urll = await ress.data[0].url    
await conn.sendFile(m.chat, urll, 'error.mp4', '*Aqui esta tu video*', m)     
} catch (err3) {
console.log('3 ' + err3)    
try {
...
let ur = result.url    
await conn.sendFile(m.chat, ur, 'error.mp4', '*Aqui esta tu video*', m)}
} catch (err4) {
console.log('4 ' + err4)    
try { 
...
await conn.sendFile(m.chat, videovio, `error.mp4`, '*Aqui esta tu video*', m)
} catch (err5) {
console.log('5 ' + err5)    
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', '*Aqui esta tu video*', m)         
} catch (err6) {
console.log('6 ' + err6)    
try {
...
console.log('7 ' + err7)    
throw `*⚠️ Error, vuelva a intentarlo*`
}}}}}}}}}
