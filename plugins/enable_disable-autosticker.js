...
if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return await this.sendButton(m.chat, '*⚠️ El vídeo no puede durar más de 10 segundos*', wm, [['Desactivar autosticker', '/disable autosticker']], m)
let img = await q.download()
if (!img) return
...
if (stiker) {
let img = await(await fetch('https://github.com/Azami19.png')).buffer()  
await this.sendFile(m.chat, stiker, 'error.jpg', null, m, false, { contextInfo: { showAdAttribution: true }})  
//await this.sendFile(m.chat, stiker, 'error.jpg', null, m, false, { contextInfo: { mentionedJid: [m.sender], externalAdReply: { showAdAttribution: true, mediaType: 'VIDEO', mediaUrl: null, title: 'Cuentas oficiales', body: 'by azami', thumbnail: img, sourceUrl: `https://github.com/Azami19/CuriosityBot-MD1`}}})  
}}
return !0
}
...
const Format = { file: `data:video/mp4;base64,${getBase64}`, processOptions: { crop: stickerMetadata?.crop, startTime: '00:00:00.0', endTime: '00:00:10.0', loop: 0
