...
//Case Tinyurl 
if (args[1] == "tinyurl") {
	let tiny = await (await fetch(`https://api.botcahx.biz.id/api/linkshort/tinyurl?link=${args[0]}&apikey=Admin`)).json()
m.reply(pros).then(_ => conn.reply(m.chat, `${tesk}${tiny.result}`,m))
}
//Case Cuttly
if (args[1] == "cuttly") {
	let cuttly = await (await fetch(`https://api.botcahx.biz.id/api/linkshort/cuttly?link=${args[0]}&apikey=Admin`)).json()
m.reply(pros).then(_ => conn.reply(m.chat, `${tesk}${cuttly.result}`,m))
}
//Case Bitly 
if (args[1] == "bitly") {
	let bitly = await (await fetch(`https://api.botcahx.biz.id/api/linkshort/bitly?link=${args[0]}&apikey=Admin`)).json()
