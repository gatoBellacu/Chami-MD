...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗] Los comandos +18  estan desactivados en este grupo, si es administrador y desea activarlos use el comando #enable modohorny*'
let url = packgirl[Math.floor(Math.random() * packgirl.length)]
conn.sendButton(m.chat, `_🥵 Pack 2 🥵_`, author, url, [['🔄 SIGUIENTE 🔄', `/${command}`]], m)
}
handler.help = ['pack2']
