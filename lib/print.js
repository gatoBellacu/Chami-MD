...
console.log(`┏━━● 𝗖𝗨𝗥𝗜𝗢𝗦𝗜𝗧𝗬 ●━━━▶ 
┃ ${chalk.blue('%s')} ${chalk.black(chalk.cyan('%s'))}ㅤ${chalk.black(chalk.cyan('%s'))} ${chalk.magenta('%s [%s %sB]')}
┃ ${chalk.cyan('%s')} ${chalk.yellow('%s%s')} ${chalk.magenta('%s')} ${chalk.black(chalk.red('%s'))}
┗━━━━━━━━━━━━▶`.trim(),
            
me + ' ~' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : ' (𝗖𝗕-𝗠𝗗 - 𝗦𝗨𝗕 𝗕𝗢𝗧)'}`,
